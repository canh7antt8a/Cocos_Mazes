...
cc.Class({
    ...
    initLevels: function initLevels(level, idx) {
        ...
        for (var i = 1; i <= 25; i++) {
            ...
            if (i > level['total_level']) {
                ...
                //判断是否解锁
                SDK().getScore(self.bid + "_" + self.mid + "_" + lid, function (score, node, lid) {
                    // console.log("++++++++++score:",lid,score);
                    var isOpen = false;
                    if (lid <= self.lastLid + 1 || score > 0) {
                        isOpen = true;
                        self.setItem(node, score, isOpen, lid);
                    } else if (openAllLevel) {
                        self.setItem(node, score, true, lid);
                    }
    ...
    onLevelItemClicked: function onLevelItemClicked(event) {
